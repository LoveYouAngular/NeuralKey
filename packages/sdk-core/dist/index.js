...
function add2(a, b2) {
  ...
  if (!Number.isInteger(b2) || b2 < 0 || b2 > 255) {
    throw new RangeError("Number is out of Uint8 range");
  }
  return a ^ b2;
}
...
function interpolatePolynomial(xSamples, ySamples, x) {
  ...
  for (let i = 0; i < limit; i++) {
    ...
    for (let j = 0; j < limit; ++j) {
      ...
      const denom = add2(xSamples[i], xSamples[j]);
      const term = div(num, denom);
      basis = mult(basis, term);
    }
    result = add2(result, mult(ySamples[i], basis));
  }
  return result;
}
...
var AssertArgument = {
  ...
  greaterThanOrEqualTo(a, b2, message) {
    if (a < b2) {
      throw new Error(message);
    }
  },
  equalTo(a, b2, message) {
    if (a !== b2) {
      throw new Error(message);
    }
  }
};
async function split3(secret, shares, threshold) {
...
async function createShards(secret) {
  ...
  const shards = await split3(secret, TOTAL_SHARDS, THRESHOLD);
  return shards;
}
...
function loadWasmScript() {
  ...
  scriptLoadPromise = new Promise((resolve2, reject) => {
